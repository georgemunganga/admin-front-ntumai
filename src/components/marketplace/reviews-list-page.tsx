...
const rows = crudPages.marketplaceReviews.rows;
  ...
  const columns = useMemo<ColumnDef<CrudRecord>[]>(
    () => [
      ...
      { accessorKey: "owner", header: "Owner" },
      {
        accessorKey: "status",
        header: "Status",
        cell: ({ row }) => <ReviewStatus status={row.original.status} />,
      },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Reviews"
        description="Marketplace review list."
        action={
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
        ...
        <div className="mb-4 flex items-center justify-between gap-3">
          <Text className="text-sm text-gray-500">{filteredRows.length} reviews</Text>
        ...
        <div className="mt-4 flex items-center justify-between">
          <Text className="text-sm text-gray-500">
            Showing {table.getRowModel().rows.length} of {filteredRows.length} reviews
          </Text>

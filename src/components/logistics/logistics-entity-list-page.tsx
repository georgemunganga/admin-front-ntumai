...
type LogisticsEntityListPageProps = {
  breadcrumb: string[];
  eyebrow: string;
  title: string;
  description: string;
...
export default function LogisticsEntityListPage({
  breadcrumb,
  eyebrow,
  title,
  description,
  ...
  const columns = useMemo<ColumnDef<CrudRecord>[]>(
    () => [
      ...
      { accessorKey: "owner", header: "Owner" },
      {
        accessorKey: "status",
        header: "Status",
        cell: ({ row }) => <LogisticsStatus status={row.original.status} />,
      },
...
function LogisticsStatus({ status }: { status: string }) {
